 ...
 * =====================================================================================
 *
 *       Filename:  s_package.c
 *
 *    Description:	测试协议时的组包服务端 s (server).
 *
 *        Version:  1.0
 *        Created:  06/01/2018 02:09:14 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  smg 
...
int main(){  
    ...
    if(bind(sockfd,(struct sockaddr*)&my_addr,sizeof(struct sockaddr))<0){//绑定地址结构体和socket  
        ...
        while(1){  
			...
			char pkt[] = {0x68, 0x0e, 0x8e, 0x2e, 0xba, 0x08, 0x64, 0x01, 0x0a, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x14};	
			printf("pkt size is %d\n",(int)sizeof(pkt));
            printf("receive success\n");  
            send(new_fd,pkt,sizeof(pkt),0);//发送内容，参数分别是连接句柄，内容，大小，其他信息（设为0即可）   
        }  
    }  
    return 0;  
}   
